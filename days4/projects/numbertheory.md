 ...
 * [:days4/projects/padic_lseries: p-adic L-series of elliptic curves] : Christian Wuthrich and I will do a lot along these lines the week before.  Finishing this off at SAGE Days might be really interesting.
 ...
 * Implement '''Tate's algorithm over number fields''' (again, this involves porting Cremona's GPL'd MAGMA code to SAGE).
 

 * Bobby Moretti: '''Weirstrass Normal Form'''. Implement EllipticCurve_from_cubic() without using MAGMA.
 
 * '''Number Fields''' There are numerous issues with number fields in SAGE.  The KANT developers have worked very hard for literally a decade to get the design of number fields right, but the design in SAGE (1) completely ignores all the good work done by the KANT group, and just doesn't feel right.
