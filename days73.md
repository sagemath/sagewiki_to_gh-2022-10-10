 ...
 * Subgroups of SL(2,R) given from generators. Design an algorithm that given a finite set of matrices in SL(2,R) answers whether the group generated by these is
    - not discrete
    - has finite covolume (in that case gives an explicit fundamental domain in canonical form)
    - has infinite covolume (in that case gives an explicit fundamental domain)
   (Charles, Vincent)
