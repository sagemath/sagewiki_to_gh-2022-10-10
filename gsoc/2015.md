...
For this, the way to go seems to [[http://trac.sagemath.org/ticket/10224|package]] [[https://www.usna.edu/CS/qepcadweb/B/QEPCAD.html|QEPCAD]] in such a quality that it can become an "optional package". One could then use:
{{{
dnf1 = solve(abs(2*x-3)/(3*x)>(x+1)/(x-2),x)
qf1 = apply(qepcad_formula.or_, map(qepcad_formula.and_, dnf1))
dnf2 = solve((4*x+5)^2/(x-3)<x+3,x)
qf2 = apply(qepcad_formula.or_, map(qepcad_formula.and_, dnf2))
qepcad(qepcad_formula.and_(qf1, qf2), vars='(x)')
}}}
to get the solution `"0 < x < 2"`.

A further topic would be to introduce shorter syntax (infix boolean operators, inequality chains) for the above, to do the following:
{{{
sage: dnf1 _and_ dnf2
0 < x < 2
}}}
Note that `0 < x < 2` would not be a string here, but a Sage object.

Motivation: This a topic of our university's 1st year classes.
