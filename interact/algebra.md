...
```
![interact/algebra/gfan_interact.png](interact/algebra/gfan_interact.png) 
...
```
![interact/algebra/gb3d.png](interact/algebra/gb3d.png) 
...
def tbp_tracker(show_eqs = checkbox(False),a = slider(-1,1,1/100,1/100), b = slider(-1,1,1/100,1/100), h_c_skew = slider(0,.1,.001,0.0, label='Homotopy skew'), scale = slider([2.0^x for x in srange(.1,4,.025)],default = 2^1.6)):
    ...
    if show_eqs:
        ...
        for i in range(len(pols)):
            show(pols[i])
```
![interact/algebra/pathtrack.png](interact/algebra/pathtrack.png) 
